...
export function MenuDropdown({ className }: MenuDropdownProps) {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const { Menu, MenuItem, PredefinedMenuItem } = await import('@tauri-apps/api/menu');
                const items = [
                    await MenuItem.new({ id: 'task_view', text: '작업보기', action: () => alert('작업보기가 클릭되었습니다!') }),
                    await MenuItem.new({ id: 'agent_info', text: '에이전트정보', action: () => alert('에이전트정보가 클릭되었습니다!') }),
                    await MenuItem.new({ id: 'terminal_stats', text: '단말누적통계보기', action: () => alert('단말누적통계보기가 클릭되었습니다!') }),
                    await PredefinedMenuItem.new({ item: 'Separator' }),
                    await MenuItem.new({ id: 'settings', text: '환경설정', action: () => alert('환경설정이 클릭되었습니다!') }),
                    await MenuItem.new({ id: 'stats_config', text: '통계표 설정', action: () => alert('통계표 설정이 클릭되었습니다!') }),
                    await MenuItem.new({ id: 'secret_info', text: '비밀정보', action: () => alert('비밀정보가 클릭되었습니다!') }),
                    await PredefinedMenuItem.new({ item: 'Separator' }),
                    await MenuItem.new({ id: 'quit', text: '종료', action: () => alert('종료가 클릭되었습니다!') }),
                ];
    ...
    const openMenu = async () => {
        if (!isMenuReady || !menuRef.current || !buttonRef.current) return;
        const rect = buttonRef.current.getBoundingClientRect();
        const { LogicalPosition } = await import('@tauri-apps/api/window');
        const pos = new LogicalPosition(rect.left, rect.bottom + 5);
        menuRef.current.popup(pos);
    };
    ...
    return (
        <button
            ...
            className={cn(
                ...
                'transition-colors duration-150',
                'bg-slate-700 hover:bg-slate-800 text-white border-r border-slate-600',
                !isMenuReady && 'opacity-50 cursor-not-allowed',
                className
            )}
            onClick={openMenu}
            onMouseDown={(e) => e.stopPropagation()}
