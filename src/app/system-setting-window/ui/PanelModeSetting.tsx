...
//             const availableWidth = containerRect.width - 40; // 좌우 여백 40px
//             const availableHeight = containerRect.height - 40; // 상하 여백 40px
...
//     }, [containerRef, config.totalWidth, config.totalHeight]);
...
//                     <div className="w-[70%] max-w-5xl h-full">
//                         <div className="bg-white rounded-lg shadow-md p-3 h-full">
//                             {/* 🆕 반응형 미리보기 컨테이너 */}
//                             <div
//                                 ref={setContainerRef}
//                                 className="w-full h-full bg-gray-50 rounded-lg p-2 flex items-center justify-center overflow-hidden"
...
//                                     }}
//                                 >
//                                     <div className="absolute top-1 right-1 text-xs bg-black text-white px-2 py-1 rounded z-10">{`${Math.round(config.totalWidth)} × ${Math.round(config.totalHeight)}px (${Math.round(scale * 100)}%)`}</div>
//                                     {hasAnyTopRegion && (
...
//                     <div><span className="font-bold">미리보기 배율:</span> <span className="text-orange-600">{`${Math.round(scale * 100)}%`}</span></div>
//                 </div>
//                 {/* --- 4. 계산 요약 정보 표시 --- */}
...
const FiveRegionPanelSettings: React.FC = () => {
    ...
    useEffect(() => {
        const updateScale = () => {
            ...
            const containerRect = containerRef.getBoundingClientRect();
            const availableWidth = containerRect.width - 10; // 좌우 여백 최소화
            const availableHeight = containerRect.height - 10; // 상하 여백 최소화
    ...
    return (
        <div className="w-full h-screen bg-gray-50 flex flex-col overflow-hidden" style={{ WebkitAppRegion: 'no-drag' } as ExtendedCSSProperties}>
            ...
            <div className="flex-1 p-1 overflow-y-auto">
                {/* 🆕 가로 배치 - 62:38 비율로 더 타이트하게 조정 */}
                <div className="flex gap-2 items-start justify-center h-full">
                    {/* 첫 번째 박스 - 미리보기 (최적화된 컨테이너 크기) */}
                    <div className="w-[62%] max-w-4xl h-full">
                        <div className="bg-white rounded-lg shadow-md p-1 h-full">
                            {/* 🆕 반응형 미리보기 컨테이너 - 패딩 최소화 */}
                            <div
                                ref={setContainerRef}
                                className="w-full h-full bg-gray-50 rounded-lg p-0.5 flex items-center justify-center overflow-hidden"
                                style={{ minHeight: '400px' }}
                            >
                                <div
                                    className="border-2 border-dashed border-red-500 bg-gray-100 rounded-lg relative flex flex-col transition-all duration-200 ease-in-out"
                    ...
                    <div className="w-[38%] h-full">
                        <div className="bg-white rounded-lg shadow-md p-1.5 h-full flex flex-col">
                            {/* 탭 헤더 */}
                            <div className="flex mb-2">
                                <button
                                    onClick={() => setActiveTab('regions')}
                                    className={`flex-1 py-1 px-2 text-xs font-bold rounded-l-md transition-colors ${activeTab === 'regions'
                                ...
                                <button
                                    ...
                                    className={`flex-1 py-1 px-2 text-xs font-bold rounded-r-md transition-colors ${activeTab === 'components'
                            ...
                            <div className="flex-1 overflow-hidden">
                                {activeTab === 'regions' ? (
                                    <div className="space-y-1 h-full overflow-y-auto pb-2">
                                        {[
                                            { key: 'region1Enabled', name: '📞 통화중 박스', color: 'teal', num: '1' },
                                            { key: 'region2Enabled', name: '📊 상태 박스', color: 'blue', num: '2' },
                                            { key: 'region3Enabled', name: '📋 폰드 정보', color: 'pink', num: '3' },
