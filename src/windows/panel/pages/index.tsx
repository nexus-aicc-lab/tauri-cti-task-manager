...
export default function PanelModePage({ onBackToLauncher }: PanelModePageProps) {
    const [currentSize, setCurrentSize] = useState({ width: 780, height: 420 });
    const [isInitialized, setIsInitialized] = useState(false);
    ...
    const adjustWindowSize = useCallback(async () => {
        ...
        try {
            ...
            const baseHeight = Math.max(MIN_HEIGHT, Math.min(baseContentHeight, MAX_HEIGHT));
            ...
            if (heightDiff > 10) {
                ...
                // Tauri API로 크기 적용 (LogicalSize 사용)
                try {
                    await invoke('apply_window_size', {
                        width: finalWidth,
                        height: finalHeight,
    ...
    }, [currentSize.height, FIXED_WIDTH, MIN_HEIGHT, MAX_HEIGHT, measureContentHeight]);
        ...
        const initializePanel = async () => {
            try {
                ...
                try {
                    ...
                    const height = Math.max(MIN_HEIGHT, Math.min(savedSize.height, MAX_HEIGHT));
    ...
    }, [FIXED_WIDTH, DEFAULT_HEIGHT, MIN_HEIGHT, MAX_HEIGHT]);
