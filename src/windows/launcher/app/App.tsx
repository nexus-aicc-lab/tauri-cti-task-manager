...
const App: React.FC = () => {
        ...
        try {
            ...
            if (update?.available) {
                ...
                if (!isAutoCheck) {
                    ...
                    toast.info(`새 버전 ${update.version}이 사용 가능합니다!`, { onClick: () => promptUpdate(update), autoClose: false });
                }
            } else {
    ...
    const promptUpdate = async (update: any) => {
        const shouldUpdate = await ask(`새 버전 ${update.version}이 사용 가능합니다.\n지금 업데이트하시겠습니까?`, { title: '업데이트 확인', okLabel: '업데이트', cancelLabel: '나중에' });
        if (shouldUpdate) await performUpdate(update);
    };
    ...
    return (
        <div style={{ backgroundColor: '#f3f4f6', minHeight: '100vh', width: '100%', position: 'relative' }}>
            <button
                onClick={() => setShowVersion(!showVersion)}
                style={{ position: 'fixed', bottom: '15px', right: '15px', backgroundColor: '#000', color: '#00ff00', padding: '8px 16px', borderRadius: '20px', fontSize: '12px', zIndex: 1000 }}>
                {isLoading ? '⏳' : '🔧'} {isLoading ? 'Loading...' : `v${appVersion}`}
            </button>

            {showVersion && !isLoading && (
                <div style={{ position: 'fixed', bottom: '60px', right: '15px', backgroundColor: '#fff', padding: '15px 20px', borderRadius: '10px', fontSize: '13px', zIndex: 1001 }}>
                    <h4>CTI Task Manager</h4>
                    <button
                        onClick={() => checkForUpdates(false)}
                        style={{ marginTop: '10px', backgroundColor: '#28a745', color: '#fff', border: 'none', padding: '6px 12px', borderRadius: '4px' }}>
                        지금 업데이트
                    </button>
                </div>
            )}
